...
void printCmd(const char *cmd, const char *desc, String &response, bool web)
{
  String line = "  ";
  line += cmd;
  for ( int i = strlen(cmd); i < COL_WIDTH; i++ )
    line += ' ';
  line += " ";
  line += desc;
  Serial.println(line);
...
bool CommandsHandler(String command, bool web_command)
{
  ...
  if ( args[0] == "help" || args[0] == "?" )
  {
    ...
    printCmd("attack", "Start attacks", response, web_command);
    printCmd("stop", "Stop current jam/attack", response, web_command);
  ...
  if ( args[0] == "attack" )
  {
    bool ret = handleAttackCommand(args, argCount, web_command, response);
    if ( web_command )
      server.send(200, "text/html", response + "ok\n");
    return ret;
  }
...
bool handleJamCommand(String *args, int argCount, bool web_command, String &response)
{
  ...
  if ( sub == "wifi" )
  {
    ...
    if ( wifiSub == "all" )
    {
      ...
      if ( web_command )
        server.send(200, "text/html", response + "ok\n");
      jamHandler(wifi_jam, String("WiFi Jamming"), bitmap_wifi_jam, false, false);
      return true;
    }
    ...
    if ( wifiSub == "smart" )
    {
      ...
      if ( web_command )
        server.send(200, "text/html", response + "ok\n");
      jamHandler(wifi_scan_jam, String("WiFi Jamming"), bitmap_wifi_jam, false, false);
      return true;
    }
    ...
    if ( wifiSub == "channel" )
    {
      ...
      display.display();
      jamHandler(wifi_channel, "Jamming WiFi channel", bitmap_wifi_jam, false, false, false, false, ch - 1, 0);
  ...
  if ( sub == "ble" )
  {
    ...
    if ( bleSub == "ad" || bleSub == "advertising" )
    {
      ...
      if ( web_command )
        server.send(200, "text/html", response + "ok\n");
      jamHandler(ble_advertising_jam, String("BLE Jamming"), bitmap_ble_jam, false, false);
      return true;
    }
    ...
    if ( bleSub == "data" )
    {
      ...
      if ( web_command )
        server.send(200, "text/html", response + "ok\n");
      jamHandler(ble_data_jam, String("BLE Jamming"), bitmap_ble_jam, true, false);
      return true;
    }
  ...
  if ( sub == "misc" )
  {
    ...
    display.println(String(stop - start));
    ...
    jamHandler(misc_jam, "Jamming from " + String(start) + " to " + String(stop), bitmap_misc_jammer, false, false, true, false, start, stop);
    updateDisplay(menu_number);
    return true;
  }
...
bool handleAttackCommand(String *args, int argCount, bool web_command, String &response)
{
  ...
  {
    printLine("", response, web_command);
    printCmd("wifi", "WiFi attacks", response, web_command);
    printLine("", response, web_command);
    return true;
  }
  ...
  if ( sub == "wifi" )
  {
    ...
    if ( wifiSub == "deauth" )
    {
      ...
      {
        printLine("", response, web_command);
        printCmd("all", "Disconnect all WiFi clients from every AP", response, web_command);
        printCmd("smart", "Scan for active channels and deauth only their", response, web_command);
        printCmd("channel <1-14>", "Target specific WiFi channel <1-14> for deauth", response, web_command);
        printLine("", response, web_command);
        return true;
      }

      String deauthSub = args[3];
      deauthSub.toLowerCase();

      if ( deauthSub == "all" )
      {
        printLine("Starting WiFi all deauther...", response, web_command);
        if ( web_command )
          server.send(200, "text/html", response + "ok\n");
      ...
      if ( deauthSub == "channel" )
      {
        if ( argCount < 4 )
        {
          printLine("Missing channel number Usage: attack wifi channel <1-14>", response, web_command);
          return true;
        }
        int ch = args[4].toInt();
        if ( ch < 1 || ch > 14 )
        ...
        printLine("Starting WiFi deauther on channel " + String(ch) + "...", response, web_command);
        if ( web_command )
          server.send(200, "text/html", response + "ok\n");
        display.clearDisplay();
        display.drawBitmap(0, 0, bitmap_wifi_deauth, SCREEN_WIDTH, SCREEN_HEIGHT, WHITE);
        display.display();
        wifi_deauth_channel(ch);
        updateDisplay(menu_number);
        return true;
      }
...
bool handleScanCommand(String *args, int argCount, bool web_command, String &response)
{
  if ( argCount == 1 )
  {
    ...
    printCmd("ble", "Scan BLE devices", response, web_command);
    printLine("", response, web_command);
    return true;
  }
  ...
  if ( sub == "wifi" )
  {
    ...
    for ( int channel = 1; channel <= 14; channel++ )
    {
      ...
      for ( int i = 0; i < count; i++ )
      {
        ...
        for ( int s = 0; s < 4 - chLen / 2; s++ )
          line += " ";
        line += chStr;
        for ( int s = 0; s < 7 - (4 - chLen / 2) - chLen; s++ )
          line += " ";
        line += "|";
  ...
  {
    printLine("", response, web_command);
    printLine("Scanning BLE devices", response, web_command);
    int devices = scan_BLE();
    printLine("Found " + String(devices) + " devices", response, web_command);
    printLine("", response, web_command);

    for ( int i = 0; i < devices - 1; i++ )
    {
      for ( int j = 0; j < devices - i - 1; j++ )
      {
        if ( BLE_RSSI[j] < BLE_RSSI[j + 1] )
        {
          std::swap(BLE_MAC[j], BLE_MAC[j + 1]);
          std::swap(BLE_RSSI[j], BLE_RSSI[j + 1]);
          std::swap(BLE_Name[j], BLE_Name[j + 1]);
          std::swap(BLE_ManufacturerData[j], BLE_ManufacturerData[j + 1]);
          std::swap(BLE_Payload[j], BLE_Payload[j + 1]);
        }
      }
    }

    printLine("+-------------------+-------+------------------------------+", response, web_command);
    printLine("|        MAC        | RSSI  |             Name             |", response, web_command);
    printLine("+-------------------+-------+------------------------------+", response, web_command);

    for ( int device = 0; device < devices; device++ )
    {
      String mac = BLE_MAC[device];
      mac.toUpperCase();

      String name = BLE_Name[device];

      String rssiStr = String(BLE_RSSI[device]);

      String line = "| ";
      line += mac;
      line += " | ";
      line += rssiStr;
      for ( int s = 0; s < 6 - rssiStr.length(); s++ )
        line += " ";
      line += "| ";
      ...
      line += name;
      for ( int s = 0; s < 29 - name.length(); s++ )
        line += " ";
      line += "|";

      printLine(line, response, web_command);
      printLine("+-------------------+-------+------------------------------+", response, web_command);
      printLine("├─ Manufacture -> " + BLE_ManufacturerData[device], response, web_command);
      printLine("├─ Payload     -> " + BLE_Payload[device], response, web_command);
      printLine("+-------------------+-------+------------------------------+", response, web_command);
    }
...
bool handleSetCommand(String *args, int argCount, bool web_command, String &response)
{
  if ( argCount == 1 )
  {
    ...
    printCmd("AP", "Access point settings", response, web_command);
  ...
  if ( param == "access_point" || param == "ap" )
  {
    if ( argCount == 2 )
    {
      printLine("", response, web_command);
